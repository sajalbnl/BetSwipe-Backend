...
import { IPosition } from '../types/models.js';
...
interface MarketGroups {
    [marketId: string]: IPosition[];
}
...
class PositionUpdater {
     ...
     * Main job execution
     */
    async execute(): Promise<PositionUpdateResult | undefined> {
        if (this.isRunning) {
        ...
        try {
            ...
            return {
                success: true,
                message: `Position update completed: ${results.updated}/${positions.length} updated`,
     ...
     */
    private groupPositionsByMarket(positions: IPosition[]): MarketGroups {
     ...
     */
    private async updatePositionGroups(marketGroups: MarketGroups): Promise<UpdateResults> {
        const results: UpdateResults = { updated: 0, failed: 0, positions: [] };
     ...
     */
    private async getCachedMarketPrice(marketId: string): Promise<PriceData> {
        ...
        // Fetch fresh price
        const priceData = await polymarketService.getMarketPrice(marketId) as PriceData;
     ...
     */
    private checkSignificantPnLChanges(positions: UpdateResults['positions']): void {
