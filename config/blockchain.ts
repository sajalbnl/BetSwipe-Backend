import { ethers, Contract, Wallet, JsonRpcProvider, FeeData } from 'ethers';
import {
    POLYGON_RPC_URL,
...
// Polygon provider
export const provider: JsonRpcProvider = new ethers.JsonRpcProvider(POLYGON_RPC_URL);
...
export const usdcContract: Contract = new ethers.Contract(
    USDC_CONTRACT_ADDRESS,
    USDC_ABI,
    provider
);
...
export const getGasPrice = async (): Promise<bigint | null> => {
    try {
        const gasPrice: FeeData = await provider.getFeeData();
...
export const formatMATIC = (amount: bigint | string | number): string => {
    return ethers.formatEther(amount);
};

export const parseMATIC = (amount: string | number): bigint => {
