...
import { TradeOutcome } from '../types/index.js';
...
class PolymarketService {
     ...
     */
    async placeMarketOrder(params: PlaceMarketOrderParams): Promise<OrderResponse> {
     ...
     * Sign order using EIP-712
     */
    async signOrder(order: any, signer: Wallet): Promise<string> {
        // EIP-712 domain
        const domain = {
            name: 'Polymarket',
            version: '1',
     ...
     * Cancel order
     */
    async cancelOrder(orderId: string, privyUserId: string): Promise<CancelOrderResponse> {
        try {
            const signer = await signerService.getSessionSigner(privyUserId);
            if (!signer) {
